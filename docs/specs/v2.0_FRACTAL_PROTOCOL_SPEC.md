...
|-------|---------|---------|--------|
| 1 | v2.0.0 | Protocol Adapters (Claude-XML) | In Progress |
| 2 | v2.1.0 | Fractal Zoom | Planned |
| 3 | v2.2.0 | Context Store v2 | Planned |
| 4 | v2.3.0 | MCP Integration | Planned |
...
```xml
<truncation_marker>
  <message>Function body truncated: 42 lines → signature only</message>
  <zoom type="function" target="apply_token_budget" budget="500"
        command="pm_encoder --zoom function=apply_token_budget --budget=500" />
</truncation_marker>
```
...
These specifications provide the complete "Molten" design for casting into Rust.

**Implemented in v0.9.0:**
- [x] `OutputFormat::ClaudeXml` enum variant
- [x] Basic `serialize_claude_xml` function
- [x] CDATA escaping for `]]>`
- [x] `get_file_group_config` for priority resolution

**To Implement in v2.0.0:**
- [ ] `--frozen` flag for deterministic output
- [ ] `--allow-sensitive` flag for privacy control
- [ ] Full metadata injection in Claude-XML
- [ ] Zoom affordances in truncation markers
