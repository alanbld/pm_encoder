 ...
 * @returns {string}
 */
export function wasm_get_lenses() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.wasm_get_lenses();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
 ...
 * ```
 * @param {string} json_files
 * @param {string} json_config
 * @returns {string}
 */
export function wasm_serialize(json_files, json_config) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(json_files, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(json_config, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.wasm_serialize(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('pm_encoder_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
